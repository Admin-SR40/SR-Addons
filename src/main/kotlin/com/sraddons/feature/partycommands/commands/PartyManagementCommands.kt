...
import com.sraddons.feature.partycommands.utils.PartyUtils
import com.sraddons.feature.partycommands.utils.errorComponent
import com.sraddons.feature.partycommands.utils.formatResponse
import com.sraddons.feature.partycommands.utils.label
import com.sraddons.feature.partycommands.utils.mc
import com.sraddons.feature.partycommands.utils.modMessage
import com.sraddons.feature.partycommands.utils.noControlCodes
import com.sraddons.feature.partycommands.utils.respond
import com.sraddons.feature.partycommands.utils.respondDisabled
import com.sraddons.feature.partycommands.utils.respondUsage
...
object PartyManagementCommands {
    ...
    private fun error(
        key: String,
        vararg args: Any,
    ) = errorComponent(key, *args)

    private val suggestMembers =
        SuggestionProvider<SharedSuggestionProvider> { _, builder ->
    ...
    fun register() {
        Commands.add(
            object : Command("warp", "Warp party members", "w") {
                    ...
                    builder.executes {
                        if (SRConfig.isCommandEnabled("warp")) {
                            if (PartyUtils.isLeader()) {
                                ...
                                respond(formatResponse(label("error"), error("not_leader")))
                            }
                        } else {
        ...
        Commands.add(
            object : Command("allinvite", "Enable all invite", "allinv") {
                override fun build(builder: LiteralArgumentBuilder<SharedSuggestionProvider>) {
                    builder.executes {
                        if (SRConfig.isCommandEnabled("allinvite")) {
                            if (PartyUtils.isLeader()) {
                                ...
                                respond(
                                    formatResponse(
                                        label("all_invite"),
                                        Component.translatable("sraddons.pc.allinvite.enabled").withColor(0x55FF55),
                                    ),
                                )
                            } else {
                                respond(formatResponse(label("error"), error("not_leader")))
        ...
        Commands.add(
            object : Command("transfer", "Transfer party leader", "pt") {
                override fun build(builder: LiteralArgumentBuilder<SharedSuggestionProvider>) {
                    builder.then(
                        Command
                            ...
                            .executes { ctx ->
                                when {
                                    !SRConfig.isCommandEnabled("transfer") -> {
                                        respondDisabled("transfer")
                                    }

                                    !PartyUtils.isInParty -> {
        ...
        Commands.add(
            object : Command("promote", "Promote member") {
                override fun build(builder: LiteralArgumentBuilder<SharedSuggestionProvider>) {
                    builder.then(
                        Command
                            .argument("player", StringArgumentType.word())
        ...
        Commands.add(
            object : Command("demote", "Demote member") {
                override fun build(builder: LiteralArgumentBuilder<SharedSuggestionProvider>) {
                    builder.then(
                        Command
                            .argument("player", StringArgumentType.word())
        ...
        Commands.add(
            object : Command("disband", "Disband party") {
                override fun build(builder: LiteralArgumentBuilder<SharedSuggestionProvider>) {
                    builder.executes {
                        if (SRConfig.isCommandEnabled("disband")) {
                            if (PartyUtils.isLeader()) {
                                ...
                                modMessage(
                                    formatResponse(
                                        label("disband"),
                                        Component.translatable("sraddons.pc.disband.done").withColor(0x55FF55),
                                    ),
                                )
                            } else {
                                respond(formatResponse(label("error"), error("not_leader")))
        ...
        Commands.add(
            object : Command("leave", "Leave party") {
                override fun build(builder: LiteralArgumentBuilder<SharedSuggestionProvider>) {
                    builder.executes {
        ...
        Commands.add(
            object : Command("kick", "Kick member", "k") {
                override fun build(builder: LiteralArgumentBuilder<SharedSuggestionProvider>) {
                    builder.then(
                        Command
                            .argument("player", StringArgumentType.word())
                    ...
                    builder.executes {
                        respondUsage("!kick <player> [reason]")
                        Command.SINGLE_SUCCESS
                    }
                }
            },
        ...
        Commands.add(
            object : Command("kickoffline", "Kick offline members") {
                override fun build(builder: LiteralArgumentBuilder<SharedSuggestionProvider>) {
                    builder.executes {
                        if (SRConfig.isCommandEnabled("kickoffline")) {
                            if (PartyUtils.isLeader()) {
                                ...
                                respond(
                                    formatResponse(
                                        label("kickoffline"),
                                        Component.translatable("sraddons.pc.kickoffline.done").withColor(0x55FF55),
                                    ),
                                )
                            } else {
                                respond(formatResponse(label("error"), error("not_leader")))
        ...
        Commands.add(
            object : Command("kickall", "Kick all members except specified") {
                override fun build(builder: LiteralArgumentBuilder<SharedSuggestionProvider>) {
                    builder.then(
                        Command
                            .argument("exceptions", StringArgumentType.greedyString())
                            .suggests(suggestMembers)
                            .executes { ctx ->
                                if (SRConfig.isCommandEnabled("kickall")) {
                                    if (PartyUtils.isLeader()) {
                                        ...
                                        val toKick =
                                            PartyUtils.getMembers().filter { member ->
                                        ...
                                        if (toKick.isNotEmpty()) {
                                            ...
                                            respond(
                                                formatResponse(
                                                    ...
                                                    Component
                                                        .translatable(
                                                            "sraddons.pc.kickall.kicking",
                                                            toKick.size.toString(),
                                                        ).withColor(0x55FF55),
                                                ),
                                            )
                                        } else {
                                            respond(
                                                formatResponse(
                                                    label("kickall"),
                                                    Component.translatable("sraddons.pc.kickall.none").withColor(0xFF5555),
                                                ),
                                            )
                                        }
                                    } else {
        ...
        Commands.add(
            object : Command("invite", "Invite player to party", "inv") {
                override fun build(builder: LiteralArgumentBuilder<SharedSuggestionProvider>) {
                    builder.then(
                        Command
                            .argument("player", StringArgumentType.word())
