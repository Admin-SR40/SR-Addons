...
object FunCommands {
    private val eightBallResponses =
        arrayOf(
            "It is certain",
            "It is decidedly so",
            "Without a doubt",
    ...
    fun register() {
        Commands.add(
            object : Command("fun", "Fun commands") {
                override fun build(builder: LiteralArgumentBuilder<SharedSuggestionProvider>) {
                    ...
                    builder.then(
                        Command
                            ...
                            .then(
                                Command
                                    .argument("min", StringArgumentType.word())
                                    .then(
                                        Command
                                            .argument("max", StringArgumentType.word())
                                            .executes { ctx ->
                                                    ...
                                                    if (min == null || max == null) {
                                                        respond(
                                                            formatResponse(
                                                                ...
                                                                Component
                                                                    .translatable(
                                                                        "sraddons.pc.fun.random.error_range",
                                                                    ).withColor(0xFF5555),
                                                            ),
                                                        )
                                                    } else {
                                        ...
                                        if (SRConfig.isCommandEnabled("random")) {
                                            ...
                                            if (max == null || max < 1) {
                                                respond(
                                                    formatResponse(
                                                        label("error"),
                                                        Component.translatable("sraddons.pc.fun.random.error_single").withColor(0xFF5555),
                                                    ),
                                                )
                                            } else {
