...
object UtilityCommands {
    fun register() {
        Commands.add(
            object : Command("forward", "Toggle party chat forwarding") {
                override fun build(builder: LiteralArgumentBuilder<SharedSuggestionProvider>) {
                    builder.executes {
                        ...
                        val forwarding = SRConfig.settings.partyCommands.respondInPartyChat
                        val statusKey = if (forwarding) "sraddons.pc.forward.on" else "sraddons.pc.forward.off"
                        val statusColor = if (forwarding) 0x55FF55 else 0xFF5555
                        respond(
                            formatResponse(
        ...
        Commands.add(
            object : Command("reload", "Reload config") {
                override fun build(builder: LiteralArgumentBuilder<SharedSuggestionProvider>) {
                    builder.executes {
                        SRConfig.load()
                        Commands.rebuildDispatcher()
        ...
        Commands.add(
            object : Command("cd", "Start countdown", "countdown") {
                override fun build(builder: LiteralArgumentBuilder<SharedSuggestionProvider>) {
                    builder.then(
                        Command
                                ...
                                if (SRConfig.isCommandEnabled("countdown")) {
                                    ...
                                    if (seconds != null) {
                                        ...
                                        respond(
                                            formatResponse(
                                                label("error"),
                                                Component.translatable("sraddons.pc.error.invalid_time").withColor(0xFF5555),
                                            ),
                                        )
                                    }
                                } else {
                            ...
                            if (currentCountdown != null) {
                                ...
                                respond(
                                    formatResponse(
                                        ...
                                        Component
                                            .translatable(
                                                "sraddons.pc.cd.remaining",
                                                Component.literal("$timeStr$label"),
                                            ).withColor(0xFFFF55),
                                    ),
                                )
                            } else {
        ...
        Commands.add(
            object : Command("clear", "Clear countdown") {
                override fun build(builder: LiteralArgumentBuilder<SharedSuggestionProvider>) {
                    builder.executes {
                        if (SRConfig.isCommandEnabled("countdown")) {
        ...
        Commands.add(
            object : Command("note", "Send saved note to party") {
                override fun build(builder: LiteralArgumentBuilder<SharedSuggestionProvider>) {
                    builder.then(
                        Command
