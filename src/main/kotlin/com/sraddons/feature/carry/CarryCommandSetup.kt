...
internal object CarryCommandSetup {
    fun addTypeNode() =
        ClientCommands
            ...
            .then(
                ClientCommands
                    .argument("typeName", StringArgumentType.word())
                    .executes { context ->
                        ...
                        if (CarryState.types.containsKey(key)) {
                            context.source.feedback(
                                Component
                                    .translatable(
                                        "sraddons.carry.type_already_exists",
                                    ...
                                    .executes { context ->
                                        ...
                                        if (CarryState.clients.containsKey(clientKey)) {
                                            context.source.feedback(
                                                Component
                                                    .translatable(
                                                        "sraddons.carry.client_already_exists",
                                                        Component.literal(playerName).withColor(0xFF55FF),
                                                    ).withColor(0xFF5555),
                                            )
                                            return@executes 1
                                        }
                                        CarryState.clients[clientKey] =
                                            CarryClient(
        ...
        ClientCommands
            ...
            .then(
                ClientCommands
                    ...
                    .then(
                        ClientCommands
                            .argument("price", StringArgumentType.string())
                            .executes { context ->
                                if (!context.source.requireEnabled()) return@executes 1
                                CarryState.saveUndo()
                                val typeName = StringArgumentType.getString(context, "typeName")
    ...
    fun setBulkPriceNode() =
        ClientCommands
            .literal("set-bulk-price")
            .then(
                ClientCommands
                    .argument("typeName", StringArgumentType.word())
                    .suggests(suggestTypes)
                    .then(
                        ClientCommands
                            .argument("bulkPrice", StringArgumentType.string())
                            .then(
                                ClientCommands
                                    .argument("threshold", IntegerArgumentType.integer(1))
                                    .executes { context ->
                                        if (!context.source.requireEnabled()) return@executes 1
                                        CarryState.saveUndo()
                                        val typeName = StringArgumentType.getString(context, "typeName")
                                        ...
                                        if (parsed == null) {
                                            context.source.feedback(
                                                Component.translatable("sraddons.carry.price_invalid").withColor(0xFF5555),
                                            )
                                            return@executes 1
                                        }
