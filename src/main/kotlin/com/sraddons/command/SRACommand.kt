...
object SRACommand {
            ...
            val reloadNode =
                ClientCommands
                    ...
                    .executes { context ->
                        SRConfig.load()
                        try {
                            com.sraddons.feature.partycommands.commands.Commands
                                .rebuildDispatcher()
                        } catch (e: Exception) {
                                    ...
                                    context.source.sendFeedback(
                                        prefix
                                            ...
                                            .append(
                                                Component
                                                    .translatable(
                                                        "sraddons.command.update.latest",
                                                        Constants.MOD_VERSION,
                                                    ).withColor(0x55FF55),
                                            ),
                                    )
                                        ...
                                        if (TextReplacer.remove(word)) {
                                            context.source.sendFeedback(
                                                prefix
                                                    .copy()
                                                    .append(
                                                        Component.translatable("sraddons.command.rt.removed", word).withColor(0x55FF55),
                                                    ),
                                            )
                                        } else {
                                            context.source.sendFeedback(
                                                prefix
                                                    .copy()
                                                    .append(
                                                        Component.translatable("sraddons.command.rt.not_found", word).withColor(0xFF5555),
                                                    ),
                                            )
                                        }
                                        1
                                    ...
                                    context.source.sendFeedback(
                                        prefix
                                            ...
                                            .append(
                                                Component
                                                    .translatable(
                                                        "sraddons.command.rt.custom_count",
                                                        customs.size,
                                                    ).withColor(0x55FFFF),
                                            ),
                                    )
                                ...
                                if (!SRConfig.settings.general.enableStandaloneCalc) {
                                    context.source.sendFeedback(
                                        Constants
                                            ...
                                            .copy()
                                            .append(
                                                Component.translatable("sraddons.command.calc.standalone_disabled").withColor(0xFF5555),
                                            ),
                                    )
                                    return@executes 1
                                }
    ...
    private fun buildAlertNode() =
        ClientCommands
            .literal("alert")
            .executes { context ->
                val prefix = Constants.makePrefix()
                context.source.sendFeedback(
                    prefix.copy().append(Component.translatable("sraddons.command.alert.title").withColor(0x55FFFF)),
                )
                            ...
                            .then(
                                ClientCommands
                                    ...
                                    .executes { context ->
                                        ...
                                        context.source.sendFeedback(
                                            prefix
                                                ...
                                                .append(
                                                    Component
                                                        .translatable(
                                                            "sraddons.command.alert.added_default",
                                                            keyword,
                                                            subtitle,
                                                        ).withColor(0x55FF55),
                                                ),
                                        ...
                                        ClientCommands
                                            .argument("cooldown", IntegerArgumentType.integer(0))
                                            .executes { context ->
                                                val keyword = StringArgumentType.getString(context, "keyword")
                                                val subtitle = StringArgumentType.getString(context, "subtitle")
                                                ...
                                                val prefix = Constants.makePrefix()
                                                SRConfig.settings.chatAlert.entries
                                                    .add("$keyword | $subtitle | $cooldown | yes | yes")
                                                SRConfig.save()
                                                context.source.sendFeedback(
                                                    prefix
                                                        .copy()
                    ...
                    .then(
                        ClientCommands
                            ...
                            .suggests { _, builder ->
                                ...
                                val prefix = Constants.makePrefix()
                                val removed =
                                    SRConfig.settings.chatAlert.entries.removeAll {
                                ...
                                if (removed) {
                                    ...
                                    context.source.sendFeedback(
                                        prefix
                                            .copy()
                                            .append(
                                                Component.translatable("sraddons.command.alert.not_found", keyword).withColor(0xFF5555),
                                            ),
                                    )
                                }
                                1
                ...
                ClientCommands
                    ...
                    .executes { context ->
                        ...
                        if (entries.isEmpty()) {
                            context.source.sendFeedback(
                                prefix.copy().append(Component.translatable("sraddons.command.alert.no_alerts").withColor(0xAAAAAA)),
                            )
                        } else {
                            context.source.sendFeedback(
                                prefix.copy().append(
                                    Component.translatable("sraddons.command.alert.list_count", entries.size).withColor(0x55FFFF),
                                ),
                            )
                            entries.forEachIndexed { i, raw ->
                                ...
                                val ignorePrefix = parts.getOrElse(3) { "yes" }
                                val ignoreSuffix = parts.getOrElse(4) { "yes" }
                                val flags =
                                    when {
                                        ignorePrefix == "no" && ignoreSuffix == "no" -> "exact"
                                        ignorePrefix == "no" -> "prefix"
                                        ignoreSuffix == "no" -> "suffix"
                                        else -> "contains"
                                    }
                                context.source.sendFeedback(
                                    prefix.copy().append(
                                        Component.translatable("sraddons.command.alert.list_entry", i + 1, kw, sub, cd, flags),
                                    ),
                                )
                            }
                        }
                ...
                ClientCommands
                    ...
                    .executes { context ->
                        ...
                        SRConfig.save()
                        context.source.sendFeedback(
                            prefix.copy().append(Component.translatable("sraddons.command.alert.cleared").withColor(0x55FF55)),
                        )
                        1
                    },
            )

    private fun openGuiCommand(
        context: com.mojang.brigadier.context.CommandContext<net.fabricmc.fabric.api.client.command.v2.FabricClientCommandSource>,
    ): Int {
        SRConfigGui.open()
        context.source.sendFeedback(
            Constants
